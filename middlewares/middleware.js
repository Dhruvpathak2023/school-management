...
const accessRouting = async(req,res,next)=>{
	const token = req.headers.authorization
	if(!token) return res.status(401).json({message:'Authorization Failed.'})
	try {
		const decoded_token = jwt.verify(token,config.JWT_SECRET)
		
		req.role = decoded_token.role
		const current_path = req.path
		
		const access_keyword = `/${req.role}/`
		let isAllowed = false

		if(current_path.includes(access_keyword)) isAllowed = true

		if(!isAllowed)
			return res.status(403).json({message:'Unauthorized access'})
		next()
	} catch (error) {
		return res.status(401).json({message:'Authorization failed, access denied.'})
	}
}
