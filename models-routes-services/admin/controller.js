...
class AdminAuth {
  async login(req, res) {
    try {
      ...
      let role = admin.isHod? 'hod': 'admin'
      const newToken = {
        sToken: jwt.sign({ _id: (admin._id).toHexString(), role: role }, config.JWT_SECRET, { expiresIn: config.JWT_VALIDITY }),
  ...
  async createSubAdmin(req, res) {
    console.log("in admin create", 81);
    try {
      req.body = pick(req.body, ['roleId', 'name', 'username', 'email', 'phoneNumber', 'password', 'status', 'type', 'isHod'])
      console.log(req.body);
      const { username, email, phoneNumber, roleId, isHod } = req.body
