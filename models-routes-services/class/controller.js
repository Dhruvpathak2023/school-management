...
class classController {
    ...
    async addClass(req,res) {
		try{
            req.body = pick(req.body, ['name', 'status', 'standard', 'classTeacherId'])

            const teacher = TeachersModel.findOne({_id: req.body.classTeacherId, status: 'Y' }, { _id: 1 }).lean()
			...
			const data = await ClassModel.create({...req.body, classTeacherId:req.body.classTeacherId})
			return res.status(status.OK).jsonp({
				status: jsonStatus.OK,
